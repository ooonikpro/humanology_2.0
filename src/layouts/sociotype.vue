...
const onScroll = (e: Event) => {
  ...
  if (container.scrollLeft % 480 === 0) isScrollingEnd = true;

  if (isScrollingEnd && activeIndex !== 1) {
    router.push(
      appRoutes.sociotypeTab(carousel.value[activeIndex], activeTab.value.name),
    );
    activeIndex = 1;
  } else {
    activeIndex = Math.round(container.scrollLeft / container.clientWidth);
  }
